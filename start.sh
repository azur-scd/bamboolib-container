...
if [ "$(id -u)" == 0 ] ; then
    ...
    if id docker &> /dev/null ; then
        ...
        usermod -d "/home/${NB_USER}" -l "${NB_USER}" docker
    fi

    # handle home and working directory if the username changed
    if [[ "${NB_USER}" != "docker" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/${NB_USER}" ]]; then
            echo "Copying home dir to /home/${NB_USER}"
            mkdir "/home/${NB_USER}"
            cp -a /home/docker/. "/home/${NB_USER}/" || ln -s /home/docker "/home/${NB_USER}"
        fi
        # if workdir is in /home/docker, cd to /home/${NB_USER}
    ...
    if [[ "${NB_UID}" == "$(id -u docker 2>/dev/null)" && "${NB_GID}" == "$(id -g docker 2>/dev/null)" ]]; then
        ...
        if [[ "${STATUS}" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                sed -e "s/^docker:/nayvoj:/" /etc/passwd > /tmp/passwd
